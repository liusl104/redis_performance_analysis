...
func (d *Decoder) Zadd(key []byte, score float64, member []byte) {
	...
	if d.currentInfo.Encoding == "skiplist" {
		if d.currentEntry != nil && containsNonPrintableOrNonASCII(d.currentEntry.FieldOfLargestElem) {
			d.currentEntry.FieldOfLargestElem = ""
		}
...
func containsNonPrintableOrNonASCII(s string) bool {
	for _, r := range s {
		// 检查是否为不可打印字符或者非 ASCII 字符
